...
const FeatureArea = () => {
   ...
   const currentItems = tours.slice(itemOffset, itemOffset + itemsPerPage);

   const startOffset = itemOffset + 1;
   const endOffset = Math.min(itemOffset + itemsPerPage, tours.length);
   ...
   return (
      <div className="tg-listing-grid-area mb-85 pt-80">
         <div className="container">
            <div className="row">
               <FeatureSidebar setTours={setTours} />
                     ...
                     <div className="tg-listing-grid-item">
                        <div className={`row list-card ${isListView ? 'list-card-open' : ''}`}>
                            {currentItems.map((tour) => (
                               <div key={tour.id} className="col-xxl-4 col-xl-6 col-lg-6 col-md-6 tg-grid-full">
                                  <div className="tg-listing-card-item tg-listing-2-card-item mb-25">
                                     <div className="tg-listing-card-thumb tg-listing-2-card-thumb fix p-relative">
                                        <Link to={`/tours/${tour.slug}`}>
                                           <img className="tg-card-border w-100" src={tour.featured_image} alt="tour" />
                                           {tour.tag && <span className="tg-listing-item-price-discount shape">{tour.tag}</span>}
                                           {tour.featured && <span className="tg-listing-item-price-discount shape-3">
                                       ...
                                       <div className="tg-listing-2-mask">
                                        ...
                                        <div className="tg-listing-item-wishlist">
                                           <a onClick={() => handleAddToWishlist(tour)} style={{ cursor: "pointer" }}>
                                     ...
                                     <div className="tg-listing-card-content p-relative">
                                        <div className="tg-listing-2-price-wrap text-center">
                                           <div className="tg-listing-2-price">
                                              <span className="new">€{tour.price_eur}</span>
                                              <span className="shift">/person</span>
                                           </div>
                                        </div>
                                        <h4 className="tg-listing-card-title"><Link to={`/tours/${tour.slug}`}>{tour.title_en}</Link></h4>
                                        <div className="tg-listing-card-review mb-5">
                                           <Rating initialValue={tour.review} size={16} readonly={true} />
                                           <span className="tg-listing-rating-percent">({tour.review} Rating)</span>
                                        </div>
